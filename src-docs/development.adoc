...
* It should be possible to run analysis on developers machines, integrated with IDE preferably.
* Coding style and architecture evolve in time, therefore is should be easy to change and adapt the rules.
* The rules definitions should be maintained centrally - so that all team members share exactly the same settings.
* Results of the analysis should be easy to find and easy to understand, visual preferably.
* It should be possible to view static code analysis results for deltas, e.g. what issues were introduced in the last commit. 
* It should be possible to notify the team if some crucial rules are violated. 
